...
public class OutStatement extends CTypeStatement implements FreeAndColumnarStatement {

	private boolean constantLock;
	private Factor dataArea;

	public OutStatement(Opcode opcode) {
		super(opcode);
	}
	...
	public void buildFreeContextStatement(ParseTree parseTree) {
		...
		Factor factor = FreeContextAnalyzer.getFreeContextFactor(parseTree.getChild(identifierContext));

		this.constantLock = factor.getValue().toUpperCase().contentEquals("*LOCK");
